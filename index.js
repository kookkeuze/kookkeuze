...
function renderShoppingList() {
  ...
  const groups = IngredientPicker.groupByCategory(open, item => item.name);
  let html = groups
...
async function openIngredientPickerForRecipe(recipeUrl, recipeTitle) {
  ...
  IngredientPicker.showLoading(title);
  try {
    const data = await fetchRecipeIngredients(recipeUrl);
    const ingredients = Array.isArray(data.ingredients) ? data.ingredients.filter(Boolean) : [];
    IngredientPicker.open({
      ...
      onConfirm: addChosenIngredientsToList
    });
  } catch (err) {
    console.error(err);
...
// Wordt aangeroepen als je in het kiesvenster op 'Toevoegen' tikt. Geeft false
// terug bij een fout, zodat het venster blijft staan en je het opnieuw kunt
// proberen zonder je uitgevinkte regels kwijt te raken.
async function addChosenIngredientsToList(chosen) {
  try {
...
document.addEventListener('keydown', e => {
  ...
  // Het kiesvenster sluit zichzelf al; de boodschappenlijst eronder blijft dan
  // staan, zodat een Escape niet twee vensters tegelijk wegklikt.
  if (IngredientPicker.isOpen()) return;
  if (shoppingListModal && !shoppingListModal.classList.contains('hidden')) {
